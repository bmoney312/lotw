    ...
    for row in players:
        ...
        if email_result is True:
            ...
            if smtp_relay is None:
                logger.error("Error establishing SMTP connection with {}".format(mail_host))
                conn.close()
                return response(200, 'text/html', build_html("Picks for week {} send failed for some players ({}).".format(week, player_id)))
            ...
            if email_result is True:
                ...
                smtp_relay.close()
                conn.close()
                return response(504, 'text/html', build_html("Picks for week {} send failed for some players ({}).".format(week, player_id)))
